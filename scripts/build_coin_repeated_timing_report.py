#!/usr/bin/env python3
"""Aggregate repeated exclusive LLaVA selector timing after warm-up."""
...
METHODS = ("full", "protected", "target", "scope", "coin")
METRICS = (
    "mean_vision_ms",
...
def main() -> None:
    ...
    parser.add_argument(
        "--runs-root",
        type=Path,
        default=ROOT / "runs" / "efficiency" / "coin_repeated_exclusive",
    )
    ...
    write_csv(runs_root / "coin_repeated_timing_runs.csv", detail)
    write_csv(runs_root / "coin_repeated_timing_summary.csv", summary)
    (runs_root / "coin_repeated_timing_report.md").write_text(
...
def summarize(detail: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for method in METHODS:
        ...
        row: dict[str, Any] = {
            ...
            "keep_ratio": statistics.mean(item["keep_ratio"] for item in group),
        }
        for metric in METRICS:
...
def markdown(detail: list[dict[str, Any]], summary: list[dict[str, Any]], warmup: int) -> str:
    lines = [
        ...
        "",
        "Three fresh-process repetitions use the same first 100 probes from the image-disjoint TextOCR-Hard confirmation split, "
