...
MODEL_PATH="${MODEL_PATH:-OpenGVLab/InternVL3_5-8B-HF}"
...
run_recap() {
  ...
  if [[ "${TRUST_REMOTE_CODE}" == "1" ]]; then
    pilot_args+=(--trust-remote-code)
  fi
  if [[ "${LOW_CPU_MEM_USAGE}" == "1" ]]; then
    pilot_args+=(--low-cpu-mem-usage)
  fi
  if [[ -n "${ATTN_IMPLEMENTATION}" ]]; then
    pilot_args+=(--attn-implementation "${ATTN_IMPLEMENTATION}")
  fi
  if [[ -n "${LIMIT}" ]]; then
